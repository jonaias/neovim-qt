...
NeovimRequest* NeovimConnector::startRequestUnchecked(const QString& method, uint32_t argcount)
{
    ...
    msgpack_pack_str(&m_pk, utf8.size());
    msgpack_pack_str_body(&m_pk, utf8.constData(), utf8.size());
...
{
	qDebug() << __func__ << raw;
    msgpack_pack_str(&m_pk, raw.size());
    msgpack_pack_str_body(&m_pk, raw.constData(), raw.size());
}
...
void NeovimConnector::addClasses(const msgpack_object& ctable)
{
	...
	for (uint32_t i=0; i<ctable.via.array.size; i++) {
		if ( ctable.via.array.ptr[i].type != MSGPACK_OBJECT_STR ) {
			setError( UnexpectedMsg,
